...
// ======== Helper chuyển enum sang string ========
...
string infantryTypeToString(InfantryType t) {
    switch (t) {
...
Configuration::Configuration(const string& filepath)
    : num_rows(0), num_cols(0), eventCode(0),
      liberationUnits(nullptr), liberationUnitsCount(0),
      ARVNUnits(nullptr), ARVNUnitsCount(0)
{
    ...
    while (getline(fin, line)) {
        ...
        // Nếu đề yêu cầu, đọc thêm arrayX, units, ...
    }
    fin.close();
}
...
string Configuration::str() const {
    ostringstream oss;
    oss << "Configuration[num_rows=" << num_rows
        << ",num_cols=" << num_cols
        << ",arrayForest=[]"
        << ",arrayRiver=[]"
        << ",arrayFortification=[]"
